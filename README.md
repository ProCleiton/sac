...
sac plugins install                         # install the canonical plugins (superpowers, rtk, openspec)
...
## Canonical plugins: superpowers + RTK + OpenSpec

Since v27, **superpowers** (skills), **RTK** (output-filtering CLI) and
**OpenSpec** (spec-driven changes CLI) are canonical plugins **managed by SAC
itself** — own clones in `$SAC_HOME/plugins/<name>/` and binaries in
`$SAC_HOME/bin/` (`SAC_HOME` defaults to `~/.sac`), each pinned to a ref.
Inside a SAC pipeline, only the SAC-managed copy matters — pre-existing
harness installs are ignored.

```bash
sac plugins install    # clone at the pinned ref + materialize bins (rtk, openspec)
sac plugins update     # fetch + checkout the pinned ref (--check: pin vs upstream, read-only)
sac plugins status     # installed? current ref? bin present?
sac plugins uninstall  # remove $SAC_HOME/plugins and $SAC_HOME/bin (asks confirmation)
```

Injection into agents (`sac up`) is automatic and data-driven
(`sac/harness_adapters.py`):

- every pane gets `$SAC_HOME/bin` at the **front of PATH** — SAC binaries win
  over any external install;
- `kimi` agents get `--skills-dir $SAC_HOME/plugins/superpowers/skills` (only
  when superpowers is installed — the flag replaces kimi's auto-discovery);
- `opencode` and `mimo` agents always get `--pure` (no external plugins);
- `claude` agents get `--bare --plugin-dir $SAC_HOME/plugins/superpowers`
  (only with superpowers installed — it is a Claude plugin);
- `copilot` agents get the env `COPILOT_SKILLS_DIRS` pointing to the SAC
  skills; `codex` agents get `-c skills.config=[...]` (both only with
  superpowers installed);
- harnesses without an adapter (gemini, aider, goose, ...) get only the PATH
  and the skills pointer in the contract;
- contracts generated by `sac init` carry a "Stack canônica SAC" section:
  RTK mandatory for verbose commands, pointer to the SAC skills, OpenSpec for
  specs/changes (leader + documentation) — it describes what's already
  available, never how to install it.

`sac doctor` checks each canonical plugin (clone at the pinned ref, bin
present) and warns with `sac plugins install` when something is missing; the
post-init checklist includes the install step.
